...
internal sealed class NotificationConfigurationHealthCheck(
    ...
    IHostEnvironment environment) : IHealthCheck
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        ValidateRequiredValue(failures, "Email:Host");
        ValidateRequiredValue(failures, "Email:Port");
        ValidateRequiredValue(failures, "Email:Username");
        ValidateRequiredValue(failures, "Email:Password");
        ValidateRequiredValue(failures, "Email:From");
        ValidateRequiredValue(failures, "Email:DisplayName");

        return failures.Count == 0
            ? Task.FromResult(HealthCheckResult.Healthy("Конфигурация notification готова."))
            : Task.FromResult(HealthCheckResult.Unhealthy("Конфигурация notification не готова к production.", data: failures));
    }
