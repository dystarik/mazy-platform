namespace MazyPlatform.Service.Notification;
    ...
    IIntegrationEventDispatcher dispatcher) : BackgroundService
{
    ...
    private async Task HandleMessageAsync(IChannel channel, BasicDeliverEventArgs ea, CancellationToken cancellationToken)
    {
        ...
        using (LogContext.PushProperty("TraceId", traceId))
        {
            ...
            try
            {
                await _dispatcher.DispatchAsync(routingKey, json, cancellationToken);
    ...
    [LoggerMessage(EventId = 3, Level = LogLevel.Information, Message = "RabbitMQ consumer запущен. Ожидание сообщений.")]
    ...
    [LoggerMessage(EventId = 6, Level = LogLevel.Error, Message = "Ошибка обработки сообщения. RoutingKey: {RoutingKey}.")]
