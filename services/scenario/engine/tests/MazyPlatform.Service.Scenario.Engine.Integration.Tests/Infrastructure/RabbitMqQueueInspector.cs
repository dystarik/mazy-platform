...
public sealed class RabbitMqQueueInspector : IDisposable
{
    ...
    public async Task AssertNoNewReadyMessagesAsync(string queueName, int previousReadyCount, TimeSpan timeout)
    {
    ...
    {
        using var response = await _client.DeleteAsync($"/api/queues/%2F/{Uri.EscapeDataString(queueName)}/contents");
        response.EnsureSuccessStatusCode();
    }
