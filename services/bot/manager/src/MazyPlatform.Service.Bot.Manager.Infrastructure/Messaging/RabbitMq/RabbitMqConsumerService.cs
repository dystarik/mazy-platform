    ...
    ILogger<RabbitMqConsumerService> logger) : BackgroundService
{
    ...
    private async Task HandleMessageAsync(IChannel channel, BasicDeliverEventArgs ea, CancellationToken cancellationToken)
    {
        ...
        using (logger.BeginScope(new Dictionary<string, object>(StringComparer.Ordinal) { ["TraceId"] = traceId }))
        {
            ...
            try
            {
                ...
                LogMessageProcessed(routingKey);
            }
            catch (Exception ex)
            {
                LogMessageError(ex, routingKey);
    ...
    #region Logging
    [LoggerMessage(1, LogLevel.Information, "Остановка RabbitMQ consumer.")]
    private partial void LogStopping();

    [LoggerMessage(2, LogLevel.Information, "RabbitMQ consumer запущен.")]
    private partial void LogStarted();

    [LoggerMessage(3, LogLevel.Information, "Подписка на очередь '{QueueName}', routing key '{RoutingKey}'.")]
    private partial void LogListeningQueue(string queueName, string routingKey);

    [LoggerMessage(4, LogLevel.Information, "Сообщение получено. RoutingKey: '{RoutingKey}'.")]
    private partial void LogMessageReceived(string routingKey);

    [LoggerMessage(5, LogLevel.Information, "Сообщение обработано. RoutingKey: '{RoutingKey}'.")]
    private partial void LogMessageProcessed(string routingKey);

    [LoggerMessage(6, LogLevel.Error, "Ошибка обработки сообщения. RoutingKey: '{RoutingKey}'.")]
    private partial void LogMessageError(Exception exception, string routingKey);
    #endregion
}
