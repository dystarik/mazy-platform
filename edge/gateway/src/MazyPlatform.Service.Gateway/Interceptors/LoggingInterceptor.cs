...
internal sealed partial class LoggingInterceptor(ILogger<LoggingInterceptor> logger) : Interceptor
{
    ...
    #region Logging
    [LoggerMessage(1, LogLevel.Information, "gRPC запрос gateway {RequestStatus}: {Method}")]
    private partial void RequestStarted(string requestStatus, string method);

    [LoggerMessage(2, LogLevel.Information, "gRPC запрос gateway {RequestStatus}: {Method} | Status: {Status} | Elapsed: {ElapsedMs}ms")]
    private partial void RequestCompleted(string requestStatus, string method, StatusCode status, long elapsedMs);

    [LoggerMessage(3, LogLevel.Warning, "gRPC запрос gateway {RequestStatus}: {Method} | Status: {Status} | Elapsed: {ElapsedMs}ms")]
    ...
    [LoggerMessage(4, LogLevel.Error, "gRPC запрос gateway {RequestStatus}: {Method} | Elapsed: {ElapsedMs}ms")]
